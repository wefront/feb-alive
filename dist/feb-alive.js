...
var supportHistoryState = function () {
  ...
  history.replaceState(_Object$assign__default['default']({
    _debug: 'ok'
  }, history.state), null);
  ...
  if (!history.state) {
    ...
    var originState = _Object$assign__default['default']({}, history.state);
...
function getLocation(base, mode) {
  ...
  if (base && path.indexOf(base) === 0) {
    path = _sliceInstanceProperty__default['default'](path).call(path, base.length);
  }
...
function remove(arr, item) {
  if (arr.length) {
    ...
    if (index > -1) {
      return _spliceInstanceProperty__default['default'](arr).call(arr, index, 1);
    }
  }
}
/**
...
function deepClone(val) {
  if (isPlainObject(val)) {
    ...
    return _sliceInstanceProperty__default['default'](val).call(val);
  } else {
    return val;
  }
}
...
function parsePath(path) {
  ...
  if (hashIndex >= 0) {
    hash = _sliceInstanceProperty__default['default'](path).call(path, hashIndex);
    path = _sliceInstanceProperty__default['default'](path).call(path, 0, hashIndex);
  }

  var queryIndex = path.indexOf('?');

  if (queryIndex >= 0) {
    query = _sliceInstanceProperty__default['default'](path).call(path, queryIndex + 1);
    path = _sliceInstanceProperty__default['default'](path).call(path, 0, queryIndex);
  }
  ...
  if (queryStr) {
    queryStr.split('&').forEach(function (kv) {
      var _kv$split = kv.split('='),
          _kv$split2 = _slicedToArray__default['default'](_kv$split, 2),
          key = _kv$split2[0],
          val = _kv$split2[1];
...
var stringifyPrimitive = function stringifyPrimitive(v) {
  switch (_typeof__default['default'](v)) {
    case 'string':
      return v;
...
var stringifyQuery = function stringifyQuery(query) {
  ...
  _Object$keys__default['default'](query).forEach(function (key, index) {
    var val = query[key];
...
var parseUrl = function parseUrl(url) {
  ...
  var newQuery = _Object$assign__default['default']({}, location.query, query);
  ...
  return {
    ...
    methods: {
      cacheClear: function cacheClear(key) {
        ...
        if (maxPage && _keysInstanceProperty__default['default'](this).length > parseInt(maxPage)) {
          var oldKey = _keysInstanceProperty__default['default'](this)[0];

          var oldVnode = this.cache[oldKey];
          oldVnode && oldVnode.componentInstance && oldVnode.componentInstance.$destroy();
          remove(_keysInstanceProperty__default['default'](this), oldKey);
        ...
        for (var _key in this.cache) {
          if (!matches(routes, _key)) {
            ...
            remove(_keysInstanceProperty__default['default'](this), _key);
    ...
    render: function render() {
      ...
      if (!history.state || !history.state[keyName]) {
        var state = _defineProperty__default['default']({}, keyName, genKey());
      ...
      if (vnode) {
        ...
        var cache$1 = this.cache,
            keys = _keysInstanceProperty__default['default'](this);
        ...
        vnode.key = _concatInstanceProperty__default['default'](_context = "__febAlive-".concat(key, "-")).call(_context, vnode.tag);
        ...
        if (from.matched[depth] === to.matched[depth] && depth !== to.matched.length - 1) {
           ...
           * 5.父路由通过key进行复用
           */
          cache$1[key] = cache$1[key] || _keysInstanceProperty__default['default'](this)[_keysInstanceProperty__default['default'](this).length - 1];
...
var nav = (function (bus, keyName) {
  ...
  var back = function back(count, toRoute, fromRoute) {
    ...
    _spliceInstanceProperty__default['default'](routes$1).call(routes$1, routes.length - count, count);
  ...
  var replace = function replace(name, toRoute, fromRoute) {
    ...
    _spliceInstanceProperty__default['default'](routes$1).call(routes$1, routes.length - 1, 1, name);
  ...
  var reset = function reset() {
    _spliceInstanceProperty__default['default'](routes).call(routes, 0, routes.length);
  ...
  var record = function record(toRoute, fromRoute, replaceFlag) {
    ...
    if (!name) {
      ...
      var state = _defineProperty__default['default']({}, keyName, key);
    ...
    Vue.location = Vue.prototype.$location = {
      ...
      recoverMeta: function recoverMeta(from, to) {
        ...
        if (!isReplace || isReplace && isSamePage) {
          if (metaMap[key]) {
            _Object$assign__default['default'](toMeta, metaMap[key]);
      ...
      getRoutes: function getRoutes() {
        return _sliceInstanceProperty__default['default'](routes).call(routes);
      },
  ...
  resetHistory: function resetHistory() {
    if (typeof history !== 'undefined' && !hasReset) {
      ...
      history.replaceState = function (state, title, path) {
        nativeHistoryReplaceState(_Object$assign__default['default']({}, history.state, state), title, path);
      };
